...
export async function GET(
  ...
  const [{ data: commercant }, { data: association }, { data: doc }] = await Promise.all([
    admin.from("commercant").select("name_entreprise, adresse").eq("id_commercant", lot.id_commercant).maybeSingle(),
    admin.from("association").select("name_entreprise, adresse").eq("id_association", collect.id_association).maybeSingle(),
    admin.from("document_fiscal").select("numero_sequentiel").eq("id_collect", collect.id_collect).maybeSingle(),
  ]);

  const buffer = await generateRecap({
    numeroSequentiel: doc?.numero_sequentiel ?? idLot,
  ...
  const num = doc?.numero_sequentiel ?? idLot;
  return new Response(new Uint8Array(buffer), {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `attachment; filename="collecte_${num}.pdf"`,
